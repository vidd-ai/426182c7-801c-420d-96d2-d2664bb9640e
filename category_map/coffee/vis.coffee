...
Bubbles = () ->
  ...
  # so user can navigate out of category map
  window.nav = (form) ->
    #input1 = form.input1.value
    #input2 = form.input2.value
    #document.getElementById("nav_form").innerHTML += '<a href="http://nytimes.com">here again</a>'
    

  # total highlight segments in this category map
  countSegments = (data) ->
    totalSegments = 0
    data.forEach (d) ->
      d.count = parseInt(d.count)
      totalSegments += d.count
    totalSegments
  ...
  chart = (selection) ->
    selection.each (rawData) ->
      ...
      #user = getParams()['u']
      #s1 = '../viewer.html?v=segSummary.html&u=' + user + '&c=File'
      #s = '<label><input type="radio"><a href="' + s1 + '">File map</a></label>'
      #d3.select("#nav").html(s);
  ...
  updateLabels = () ->
    ...
    #label.each((d) -> d.dx = this.getBoundingClientRect().width + margin.left)
  ...
  updateActive = (id) ->
    ...
    if id.length > 0
      d3.select("#status").html("<span class='text'>The word <span class=\"active\">#{id}</span> is now active</span>")
      d3.select("#status").html(s);
    else
...
# of our chart with its data and div selector
# specified
# ---
...
texts = [
  {key:"labels",file:"labels.csv",name:"Descriptions in your Vidd.ai category map"}
  {key:"persons",file:"persons.csv",name:"Persons in your Vidd.ai category map"}
  {key:"places",file:"places.csv",name:"Places in your Vidd.ai category map"}
]
...
$ ->
  ...
  $("#input_category").attr('checked', 'checked');
  d3.selectAll("input")
    .on "change", () ->
